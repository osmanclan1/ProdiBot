...
try:
    ...
    DYNAMO_STATE_GSI_NAME = 'StatusandTime' # GSI: status (PK), next_action_time (SK)
    state_table = dynamodb.Table(DYNAMO_STATE_TABLE_NAME)
    
    print(f"Successfully connected to DynamoDB tables: {DYNAMO_REMINDER_TABLE_NAME} and {DYNAMO_STATE_TABLE_NAME}")
...
def get_task_context(user_id):
    """Fetches the active task state from DynamoDB."""
    try:
        response = state_table.get_item(Key={'user_id': str(user_id)})
        return response.get('Item', None)
    except Exception as e:
        print(f"[Log] ERROR fetching context for user {user_id}: {e}")
        return None

def add_memory_message(user_id, role, content):
    """Adds a message to a user's conversation log in DynamoDB."""
    try:
        ...
        if context and len(context.get('messages', [])) > MAX_MEMORY_MESSAGES:
            # Trim the *oldest* message (the first one in the list)
            state_table.update_item(
                Key={'user_id': str(user_id)},
                UpdateExpression="REMOVE messages[0]"
            )
        print(f"[Log] Added memory message for {user_id}. Role: {role}")
    except Exception as e:
        print(f"[Log] ERROR adding memory message for {user_id}: {e}")

def create_task_state(user_id, task, initial_message_content):
    """Creates a new state item in DynamoDB when a reminder is sent."""
    try:
        ...
        print(f"[Log] Created task state for {user_id}. First nudge at: {next_nudge_time.isoformat()}")
        return True
    except Exception as e:
        print(f"[Log] ERROR creating task state for {user_id}: {e}")
        return False
...
def calculate_next_from_rule(rule_str):
    try:
        ...
        if parts[0] != 'WEEKLY' or len(parts) != 3: print(f"[Log] Invalid rule format: {rule_str}"); return None
    ...
    except Exception as e: print(f"[Log] Error parsing rule {rule_str}: {e}"); return None
...
async def add_reminder_to_db(author_id, channel_id, remind_time, task, is_recurring=False, recurrence_rule=None):
    try:
        ...
        reminders_table.put_item(Item=item_to_put)
...
async def on_message(message):
            ...
            try:
                ...
                for row in reader:
                    try:
                        ...
                        if not due_datetime: print(f"[Log] Failed to parse date: {datetime_str}"); errors_found += 1; continue
                    ...
                    except Exception as e: print(f"[Log] Error processing CSV row: {e} (Row: {row})"); errors_found += 1
    ...
    if isinstance(message.channel, discord.DMChannel) and not message.content.startswith("!"):
        ...
        print(f"[DM USER] {user_id}: {message.content}")
        add_memory_message(user_id, "user", message.content)
        ...
        if current_status == "WAITING_FOR_REPLY":
            ...
            if status == "[TASK_DONE]":
                ...
                state_table.delete_item(Key={'user_id': str(user_id)})
                print(f"[Log] Task complete for user {user_id}. State deleted.")
                ...
                state_table.update_item(
                    ...
                    ExpressionAttributeValues={
                        ...
                        ':dt': new_despawn_time.isoformat() # Reset the 24h clock
                    }
                )
                print(f"[Log] User {user_id} not done. Next check-in at {next_action_time.isoformat()}")
...
# --- Background Loops (Refactored for DB) ---
...
async def check_reminders():
    ...
    try:
        response = reminders_table.query(
            IndexName=DYNAMO_REMINDER_GSI_NAME,
        ...
        for reminder in due_reminders:
            ...
            try:
                ...
                if user:
                    # Check if user *already* has an active task state.
                    if get_task_context(author_id):
                        print(f"[Log] User {author_id} already has an active task. Deleting duplicate DB entry.")
                        sent_successfully = True # Mark as "sent" to delete it
                    else:
                        try:
                            ...
                            # --- Create the persistent state in the *other* table ---
                            create_task_state(author_id, task, reply_content)
                            ...
                            try:
                                ...
                                print(f"[Log] Public fallback failed: {e}")
                        except Exception as e:
                            print(f"[Log] Unknown error trying to DM user: {e}")
                
                if sent_successfully:
                    # Delete the reminder *after* it's been sent (or skipped)
                    reminders_table.delete_item(Key={'user_id': str(author_id), 'reminder_id': reminder_id})
                    print(f"[Log] Deleted reminder {reminder_id} from {DYNAMO_REMINDER_TABLE_NAME}.")
                    ...
                    print(f"[Log] Failed to send reminder {reminder_id}. Will retry next loop.")

            except Exception as e:
                print(f"[Log] CRITICAL error in check_reminders sub-loop: {e}")
                try:
                    reminders_table.delete_item(Key={'user_id': reminder['user_id'], 'reminder_id': reminder['reminder_id']})
        ...
        print(f"[Log] An unexpected error occurred querying DynamoDB (Reminders): {e}")
...
async def check_followups():
    ...
    try:
        ...
        response_snooze = state_table.query(
            IndexName=DYNAMO_STATE_GSI_NAME,
        ...
        response_ghost = state_table.query(
            IndexName=DYNAMO_STATE_GSI_NAME,
            KeyConditionExpression='#s = :s AND #nat <= :now', # Use placeholder #nat
            ExpressionAttributeNames={
                '#s': 'status',
...
async def setreminder(ctx, users: commands.Greedy[discord.User], time_str: str, *, task: str):
    ...
    try:
        ...
        for user in users:
            if await add_reminder_to_db(user.id, ctx.channel.id, remind_time, task): success_users.append(user.mention)
...
def find_reminder_by_id(short_id):
    """Scans the reminders_table for a matching short_id."""
    try:
...
async def deletereminder(ctx, short_id: str):
    ...
    try:
        reminders_table.delete_item(Key={'user_id': item['user_id'], 'reminder_id': item['reminder_id']})
...
async def updatetime(ctx, short_id: str, time_str: str):
    ...
    try:
        ...
        reminders_table.put_item(Item=item)
...
async def memdump(ctx, user: discord.User):
    ...
    context = get_task_context(user.id)
    if not context:
...
async def memclear(ctx, user: discord.User):
    ...
    try:
        state_table.delete_item(Key={'user_id': str(user.id)})
