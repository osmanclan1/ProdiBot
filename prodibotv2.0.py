...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("prodibot.log"), # Log to a file
...
# --- Import our shared database logic ---
import db_utils
...
# --- DynamoDB Setup ---
log.info("bot.py is referencing DB tables from db_utils.")
...
if not DISCORD_TOKEN or not OPENAI_API_KEY:
    log.critical("="*50); log.critical("ERROR: DISCORD_TOKEN or OPENAI_API_KEY is missing."); log.critical("="*50); exit()
try:
    client = OpenAI(api_key=OPENAI_API_KEY)
except Exception as e:
    log.critical(f"Error initializing OpenAI client: {e}"); exit()
...
async def get_task_status_from_ai(user_message, user_id):
    ...
    try:
        completion = await asyncio.to_thread(
            ...
            model="gpt-4.1-nano", messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}],
            max_tokens=5, temperature=0.0
        )
        response_text = completion.choices[0].message.content.strip()
        if response_text == "[TASK_DONE]":
            log.info("[Log] AI classified as: [TASK_DONE]"); return "[TASK_DONE]"
        else:
            log.info("[Log] AI classified as: [TASK_NOT_DONE]"); return "[TASK_NOT_DONE]"
    except Exception as e:
        log.error(f"[Log] ERROR calling OpenAI for classification: {e}"); return "[TASK_NOT_DONE]"
...
async def get_memory_chat_reply(user_id):
    ...
    if not context:
        log.error(f"[Log] ERROR: get_memory_chat_reply called for user {user_id} with no context.")
        return None
    ...
    if not instruction:
        log.error(f"[Log] ERROR: User {user_id} has context but no task instruction.")
        return None
    ...
    try:
        ...
        log.info(f"[Log] OpenAI chat reply: {reply[:50]}...")
        return reply
    except Exception as e:
        log.error(f"[Log] ERROR calling OpenAI for chat reply: {e}")
        return None

# --- Bot Events ---
@bot.event
async def on_ready():
    log.info(f'Logged in as {bot.user.name} (ID: {bot.user.id})'); log.info('Bot is ready.')
    ...
    if isinstance(message.channel, discord.DMChannel) and not message.content.startswith("!"):
        ...
        if current_status == "WAITING_FOR_REPLY":
            ...
            if status == "[TASK_DONE]":
                ...
                # --- FIX: Must 'await' and run in thread ---
    ...
    try:
        ...
        for reminder in due_reminders:
            ...
            try:
                ...
                # FIX: Must 'await' this async function
...
async def before_check_reminders():
    ...
    log.info("Reminder check loop is starting.")
...
async def check_followups():
    ...
    try:
        ...
        response_snooze = await asyncio.to_thread(
            ...
            ExpressionAttributeNames={'#s': 'status', '#nat': 'next_action_time'},
            ExpressionAttributeValues={':s': 'WAITING_TO_REMIND', ':now': now_iso}
        )
        
        for item in response_snooze.get('Items', []):
            user_id = int(item['user_id'])
            task = item['task']
            log.info(f"Snooze over for user {user_id}. Nudging for task: {task}")
            try:
                ...
                if user:
                    ...
                    await asyncio.to_thread(
                        ...
                        Key={'user_id': str(user_id)},
                        UpdateExpression="SET #s = :s, #nat = :nat, #dt = :dt",
                        ExpressionAttributeNames={'#s': 'status', '#nat': 'next_action_time', '#dt': 'despawn_time'},
                        ExpressionAttributeValues={':s': 'WAITING_FOR_REPLY', ':nat': next_nudge_time.isoformat(), ':dt': new_despawn_time.isoformat()}
                    )
            except Exception as e:
                log.error(f"Error processing snooze for {user_id}: {e}")
                await asyncio.to_thread(db_utils.state_table.delete_item, Key={'user_id': str(user_id)})
        ...
        response_ghost = await asyncio.to_thread(
            ...
            KeyConditionExpression='#s = :s AND #nat <= :now',
            ExpressionAttributeNames={'#s': 'status', '#nat': 'next_action_time'},
            ExpressionAttributeValues={':s': 'WAITING_FOR_REPLY', ':now': now_iso}
        )
        ...
        for item in response_ghost.get('Items', []):
            ...
            if despawn_time <= now:
                log.info(f"Despawn time reached for user {user_id} on task: {task}. Deleting state.")
                try:
                    user = await bot.fetch_user(user_id)
                    if user:
                    ...
                    log.error(f"Error sending final despawn message to {user_id}: {e}")
                    await asyncio.to_thread(db_utils.state_table.delete_item, Key={'user_id': str(user_id)})
                
                continue 

            log.info(f"Ghost-nudge for user {user_id} for task: {task}")
            try:
                user = await bot.fetch_user(user_id)
                if user:
                    phrase = random.choice(RE_REMINDER_PHRASES)
                ...
                log.error(f"Error ghost-nudging {user_id}: {e}")
                await asyncio.to_thread(db_utils.state_table.delete_item, Key={'user_id': str(user_id)})

    except Exception as e:
        log.critical(f"An unexpected error occurred querying DynamoDB (State): {e}")


@check_followups.before_loop
async def before_check_followups():
    await bot.wait_until_ready()
    log.info("Follow-up check loop is starting.")

# --- Bot Commands (Corrected) ---
...
async def deletereminder(ctx, short_id: str):
    item, error = await db_utils.find_reminder_by_id(short_id)
...
async def updatetime(ctx, short_id: str, time_str: str):
    ...
    try:
        ...
        await asyncio.to_thread(db_utils.reminders_table.delete_item, Key={'user_id': item['user_id'], 'reminder_id': item['reminder_id']})
        item['remind_time_utc'] = new_remind_time_iso 
        if 'is_recurring' in item:
            item['is_recurring'] = False; item['recurrence_rule'] = 'NONE'
        
        await asyncio.to_thread(db_utils.reminders_table.put_item, Item=item)
...
# --- Admin Memory Commands (Corrected) ---
...
async def memclear(ctx, user: discord.User):
    ...
    try:
        ...
        log.info(f"Admin {ctx.author.id} cleared state for {user.id}")
    except Exception as e:
        await ctx.send(f"An error occurred while clearing state: {e}")
        log.error(f"ERROR clearing state for {user.id}: {e}")
...
if __name__ == "__main__":
    try:
        ...
        log.critical("="*50); log.critical("ERROR: Invalid DISCORD_TOKEN."); log.critical("="*50)
    except Exception as e:
        log.critical(f"An error occurred while running the bot: {e}")